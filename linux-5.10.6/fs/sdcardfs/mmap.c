...
static vm_fault_t sdcardfs_fault(/*struct vm_area_struct *vma,*/ struct vm_fault *vmf)
{
	int err;
	struct file *file;
	...
	file = (struct file *)vmf->vma->vm_rtk_private_data;
#else
...
static vm_fault_t sdcardfs_page_mkwrite(/*struct vm_area_struct *vma,*/
			       struct vm_fault *vmf)
{
	int err = 0;
	struct file *file;
